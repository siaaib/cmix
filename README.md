...
```bash
CUDA_VISIBLE_DEVICES=0 rye run python run/train.py model=CenterNet dataset=centernet feature_extractor=WaveNetSpectrogram decoder=UNet1DDecoderLSTM
```

CUDA_VISIBLE_DEVICES=3 rye run python run/train.py model=SpecTrDr dataset=seg decoder=SAKTModelDr exp_name=comb_0xf1x feature_extractor=WaveNetSpectrogramMulti split=fold_4 downsample_rate=1

CUDA_VISIBLE_DEVICES=3 rye run python run/train.py model=Spec2DCNN dataset=seg decoder=UNet1DDecoderLSTM exp_name=comb_0xf1x feature_extractor=WaveNetSpectrogram split=fold_0 downsample_rate=8

CUDA_VISIBLE_DEVICES=0 rye run python run/train.py model=SpecTr dataset=seg decoder=SAKTModel exp_name=comb_0xf1x feature_extractor=LSTMFeatureExtractor split=fold_0 downsample_rate=4
