...
class ResidualGRU(nn.Module):
    def __init__(self, d_model):
        ...
        self.LSTM = nn.GRU(d_model, d_model, num_layers=1, bidirectional=True, batch_first=True)
        self.linear1 = nn.Linear(d_model * 2, d_model * 4)
        self.linear2 = nn.Linear(d_model * 4, d_model)
...
class RNNFeatureExtractor(nn.Module):
    def __init__(
        ...
        self.residual_lstm = ResidualLSTM(hidden_size)
        self.residual_gru = ResidualGRU(hidden_size)
        self.out_chans = 1
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = self.residual_lstm(x)
        x = self.residual_gru(x)
...
class WAVELSTMFeatureExtractor(nn.Module):
    def __init__(
        self,
        in_channels: int,
        hidden_size: int,
        ...
        super().__init__()
        self.lstm_section = RNNFeatureExtractor(in_channels,
            hidden_size,
            num_layers,
            bidirectional,
            stride,
            out_size,
            rnn_type,)
        self.wave_section = WaveNetSpectrogram(in_channels,
            base_filters,
            wave_layers,
            wave_block,
            kernel_size,
            ...
            separate_channel,
            reinit,)
        self.height = self.wave_section.height
        self.out_chans = self.wave_section.out_chans + self.lstm_section.out_chans
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        # x: (batch_size, in_channels, time_steps)
        x_lstm = self.lstm_section(x)
        x_wave = self.wave_section(x)
        return torch.cat([x_lstm, x_wave], dim=1)
